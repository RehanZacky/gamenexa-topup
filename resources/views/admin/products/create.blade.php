...
@section('title', 'Tambah Produk Baru — Admin GameNexa')
@section('header-title', 'Tambah Produk Baru')
@section('header-subtitle', 'Tambahkan nominal top-up game baru secara manual')
...
<div class="max-w-4xl mx-auto">
    <div class="glass-card rounded-2xl p-6 sm:p-8 shadow-xl shadow-black/20">
        <div class="flex items-center justify-between pb-6 mb-6 border-b border-purple-900/30">
            <h2 class="font-heading font-bold text-lg text-white">Formulir Produk Top-Up</h2>
            <a href="{{ route('admin.products.index') }}" class="text-xs text-zinc-400 hover:text-white flex items-center gap-1.5 transition-colors">
        ...
        <form action="{{ route('admin.products.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <small class="block text-[11px] text-zinc-400 mt-1">Pilih kategori untuk memuat SKU dan harga modal Digiflazz secara otomatis.</small>
                ...
                <div class="md:col-span-2 p-4 rounded-xl bg-purple-950/20 border border-purple-900/40 space-y-3">
                    ...
                    <div id="skuDropdownContainer">
                        <select id="sku_select" onchange="onSkuSelected(this)"
                            ...
                            <option value="">— Silakan Pilih Kategori Game Terlebih Dahulu —</option>
                        </select>
                    ...
                    <div id="skuManualContainer" class="hidden space-y-2">
                        <input type="text" name="provider_product_code" id="provider_product_code" value="{{ old('provider_product_code') }}" required placeholder="Contoh: ML86 / FF70 / PUBG60"
                               class="w-full bg-dark-900/90 border border-purple-900/40 rounded-xl px-4 py-3 text-sm text-white font-mono placeholder-zinc-500 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500/30">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Contoh: 86 Diamonds (78 + 8 Bonus)"
                           class="w-full bg-dark-900/90 border border-purple-900/40 rounded-xl px-4 py-3 text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500/30 transition-all">
                    <small class="block text-[11px] text-zinc-400 mt-1">Otomatis sinkron ke nominal spesifik item saat memilih SKU Digiflazz.</small>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="modal_price" id="modal_price" value="{{ old('modal_price', 0) }}" readonly
                    ...
                    <small class="block text-[11px] text-zinc-400 mt-1.5 flex items-center gap-1.5">
                        ...
                        Otomatis tersinkronisasi dari harga Digiflazz saat memilih SKU di atas.
                    </small>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="selling_price" id="selling_price" value="{{ old('selling_price', 0) }}" required min="0" step="1" oninput="calcMargin()"
                               placeholder="Tentukan harga jual..."
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="2" placeholder="Contoh: Proses instan 1-5 detik, legal 100%..."
                          class="w-full bg-dark-900/90 border border-purple-900/40 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-purple-500">{{ old('description') }}</textarea>
            </div>

            <!-- Submit Buttons -->
            <div class="flex items-center justify-end gap-3 pt-6 border-t border-purple-900/30">
                <a href="{{ route('admin.products.index') }}" class="px-5 py-2.5 rounded-xl bg-dark-800 hover:bg-dark-700 text-zinc-300 font-bold text-xs transition-colors">
                    Batal
                </a>
...
<script>
        ...
        try {
            ...
            if (data.success && data.products && data.products.length > 0) {
                ...
                data.products.forEach(p => {
                    ...
                    opt.value = p.sku;
                    opt.dataset.nominal = p.nominal || p.clean_name;
                    opt.dataset.name = p.nominal || p.clean_name;
                    ...
                    const statusText = p.is_active ? '' : ' [OFF/GANGGUAN]';
                    opt.textContent = `[${p.sku}] ➜ ${p.nominal || p.clean_name} — (Modal: Rp ${priceFormatted})${statusText}`;
                    select.appendChild(opt);
                });

                select.disabled = false;

                // Cek jika sebelumnya ada old SKU
                const oldSku = document.getElementById('provider_product_code').value;
                if (oldSku) {
                    select.value = oldSku;
                    onSkuSelected(select);
                }
            } else {
            ...
            select.innerHTML = '<option value="">— Gagal memuat SKU dari Digiflazz (Gunakan Input Manual) —</option>';
            select.disabled = false;
            noticeEl.classList.remove('hidden');
    ...
    function onSkuSelected(selectElement) {
        ...
        const nominal = selectedOption.dataset.nominal || selectedOption.dataset.name || selectedOption.dataset.rawname || '';
        ...
        // Auto-sinkronkan Nama Nominal Produk langsung ke nominal spesifik produk
        const nameInput = document.getElementById('name');
        nameInput.value = nominal;
        nameInput.classList.add('ring-2', 'ring-purple-500/50', 'bg-purple-950/30');
        setTimeout(() => nameInput.classList.remove('ring-2', 'ring-purple-500/50', 'bg-purple-950/30'), 700);

        // Update dynamic badge nominal
        const nominalBadge = document.getElementById('nominalBadge');
        const nominalValueText = document.getElementById('nominalValueText');
        if (nominalBadge && nominalValueText) {
            nominalValueText.textContent = nominal;
            nominalBadge.classList.remove('hidden');
        }

        // Auto-fill Harga Modal (Digiflazz)
    ...
    // Auto-trigger on page load jika kategori sudah terpilih
